...
class Table {
 public:
   ...
   //parameter: size of the hashtable.
   ...
   //parameter: the key and value you want to insert to the hash table.
   ...
   //parameter: the key you want to lookup
   int *lookup(const std::string &key);

   // remove a pair given its key
   // false iff key wasn't present
   //the key that you want to look for and then remove the node.
 ...
 private:
   ...
   ListType* data; // the data, an array of linked list storing the data of the hash table.
   int num; // number of elements/nodes inserted
